...
/**
 * Created by BigMao on 2017/1/10. 对外api接口，详细需求和api文档查看
 * 《工业物联网盒子API接口需求(讨论后稿).docx》和《工业物联网盒子对外接口1.1.docx》
 */
@Controller
@RequestMapping("/api")
public class BaseInfoAPI {
	...
	@ResponseBody
	public JSONObject plcInfo(@RequestParam("token") String token, @RequestParam("device_id") String deviceId) {
		try {
	...
	public JSONObject uploadOrderInfo(@RequestParam("token") String token,
			@RequestParam(value = "device_id") String deviceId, @RequestParam("order_code") String orderCode,
	...
	@ResponseBody
	public JSONObject regInfo(@RequestParam("token") String token, @RequestParam(value = "device_id") String deviceId) {
		try {
	...
	@ResponseBody
	public JSONObject historyDataForReg(@RequestParam("token") String token, @RequestParam("reg_id") String regId,
		...
		try {
			...
			// todo
			PageInfo<GywlwHistoryItem> list = projectService.getHistoryDataForReg(regId, startTime, endTime, cursor,
					limit);
	...
	@ResponseBody
	public JSONObject rules(@RequestParam("token") String token, @RequestParam("device_id") String deviceId) {
		try {
